...
class ManagementPanelView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["shipping_methods_count"] = ShippingMethod.objects.all().count()

        context["pending_payments"] = (
            Payment.objects.all().filter(is_paid=False).count()
        )

        today = datetime.now()
        today_minus_30d = today - timedelta(days=30)

        context["orders_count_30d"] = (
            Order.objects.all().filter(created_at__gt=today_minus_30d).count()
        )
        context["revenue_30d"] = (
            Payment.objects.all()
            .filter(is_paid=True, created_at__gt=today_minus_30d)
            .aggregate(Sum("amount"))["amount__sum"]
        )

        context["recent_orders"] = (
            Order.objects.all().select_related("customer").order_by("-created_at")[:5]
        )
        context["recent_reviews"] = (
            Review.objects.all().select_related("author").order_by("-created_at")[:2]
        )
